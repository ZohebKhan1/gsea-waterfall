...
stopifnot(
  ...
  !anyNA(gsea_cardiomyocyte_vs_mesoderm),
  !anyDuplicated(gsea_cardiomyocyte_vs_mesoderm$go_term_id))

waterfall_plot <- plot_gsea_waterfall(
  gsea_cardiomyocyte_vs_mesoderm,
  direction = 'positive',
  top_n = 100L)
volcano_plot <- plot_gsea_volcano(gsea_cardiomyocyte_vs_mesoderm)
...
stopifnot(all(vapply(
  list(waterfall_plot, volcano_plot, half_volcano_plot, scatter_plot),
  inherits,
  logical(1),
  what = 'ggplot')))
stopifnot(
...
# 2.0 verify public API and failure boundaries -----------------
...
keyed_scatter_plot <- plot_gsea_nes_scatter(
  x_results,
  y_results,
  x_label = 'x',
  y_label = 'y',
  ...
  id_col = 'id',
  include_nonsignificant = TRUE)
stopifnot(nrow(keyed_scatter_plot$data) == 2L)
...
removed_functions <- c(
  ...
  'make_gsea_waterfall',
  'make_gsea_volcano',
  'make_gsea_half_volcano',
  'make_gsea_scatterplot')

stopifnot(
  all(vapply(public_functions, exists, logical(1), mode = 'function')),
  !any(vapply(removed_functions, exists, logical(1), mode = 'function')))

message('GSEA input, scientific, plotting, API, and failure-boundary contracts passed.')
